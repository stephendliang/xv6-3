...
#define moveby1(arr, i, lim) while (i < (lim-1)) {arr[i] = arr[i + 1];i+=1;}
...
found:
  ...
  memset(p->times, 0, sizeof(int) * 3);
  memset(p->ticks, 0, sizeof(int) * 3);
  p->wait_time = 0;
  ...
  for(;;){
    ...
    if (c0!=0){
      for (int i=0;i < c0;i+=1) {
        ...
        p->sched_stats[p->num_stats_used].start_tick = start;
        p->sched_stats[p->num_stats_used].duration = duration;
...
getpinfo(int pid)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    if (p->pid == pid) {
      ...
      cprintf("wait_time=%d\n", p->wait_time);
      cprintf("ticks={%d, %d, %d}\ntimes={%d, %d, %d}\n", p->ticks[0], p->ticks[1], p->ticks[2], p->times[0], p->times[1], p->times[2]);
